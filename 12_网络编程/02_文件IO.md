...
| 维度 | 回答要点 | 面试高频追问 |
| --- | --- | --- |
| 核心定义 | `文件IO` 属于网络编程，重点是Linux 并发、IPC、I/O 或网络服务中的协作机制。 | 它解决了什么问题，边界在哪里？ |
...
1. **基础：** 请定义 `文件IO`，并用本节示例说明它解决的实际问题。
...
| 对比项 | `文件IO` | 相近 IPC、I/O 或并发模型 | 选择建议 |
| --- | --- | --- | --- |
...
```cpp
// 面试考点：这里说明 `文件IO` 的职责边界，而不是重复代码字面含义。
...
- **可能的面试题：** “请结合 `文件IO` 说明设计/实现选择，并给出一个失败场景。”
