...
#define		SET_BIT(byte, i)	(byte |= 1 << (7 ^ (i)))  //ÉèÖÃÕâ¸ö×Ö½ÚµÄÖ¸¶¨Î»Îª1
#define		CLR_BIT(byte, i)	(byte &= ~(1 << (7 ^ (i)))) //ÉèÖÃÕâ¸ö×Ö½ÚµÄÖ¸¶¨Î»Îª0

typedef unsigned char	u8;
typedef u8	boolean;
typedef unsigned int	u32;

typedef struct FREQ{  //Í³¼Æ×Ö·ûÖÖÀà¼°Æµ¶È
	u8 alpha;  //×Ö·ûÖÖÀà,0-255
	u32	freq;  //×Ö·ûÆµ¶È
}FREQ;

typedef struct HUFF_TABLE{  //¹ş·òÂü±í
	FREQ freq;   //×Ö·ûÖÖÀà¼°Æµ¶È
	short leftChild;  //×óº¢×ÓÏÂ±ê
	short rightChild; //ÓÒº¢×ÓÏÂ±ê
	u8	isVisited;    //ÊÇ·ñ·ÃÎÊµÄ±ê¼Ç
	char *huffCode;   //¹ş·òÂü±àÂë
}HUFF_TABLE;
...
HUFF_TABLE *initHuffTab(FREQ *freq, int alphaCount);  //³õÊ¼»¯¹ş·òÂü±í(Ò¶×Ó½áµã)
...
void coddingFile(HUFF_TABLE *huf, FREQ *freq, int alphaCount, char *scFile, char *tgFile){
	...
	if((fpin = fopen(scFile, "r")) == NULL){
		printf("%sÎÄ¼ş´ò¿ªÊ§°Ü£¡£¡£¡", scFile);
		return;
	}
	
	if((fpout = fopen(tgFile, "w+b")) == NULL){
		printf("%sÎÄ¼ş´´½¨Ê§°Ü£¡£¡£¡", tgFile);
		return;
	}
...
void main(int argc, char **args){
	...
	if(argc <= 1 || argc > 3){
		puts("ÓÃ·¨: textFileHuff <Ô´ÎÄ¼şÃû> [Ä¿±êÎÄ¼şÃû]");
		return;
	}
