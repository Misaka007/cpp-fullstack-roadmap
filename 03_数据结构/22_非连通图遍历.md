...
**原理：** 一次 DFS/BFS 只能走到与起点连通的顶点。例：图有两个分量 `A-B-C` 和 `D-E`；从 A 进行 DFS 后，A、B、C 被标记，但 D、E 仍未访问。外层循环继续扫描顶点，发现 D 未访问后再启动一次 DFS，才会访问 D、E。外层循环负责找到每个分量的入口，内层 DFS/BFS 负责走完这个分量。
...
```text
算法 TraverseDisconnectedGraph(graph)
    将所有 visited 初始化为 false
    对每个顶点 v 循环
        如果未 visited[v]
            DFS(graph, v, visited)         // 或 BFS，完成一个连通分量
        结束如果
    结束循环
结束算法
```
...
| 维度 | 回答要点 | 面试高频追问 |
| --- | --- | --- |
| 核心定义 | `非连通图遍历` 属于数据结构与算法，关注数据表示、不变量和操作复杂度之间的取舍。 | 它解决什么问题，前置条件是什么？ |
...
1. **基础：** 请定义 `非连通图遍历`，说明它的输入、输出和核心约束。
...
- **问题：** “请说明 `非连通图遍历` 的核心不变量，并描述一个最容易出错的边界场景。”
