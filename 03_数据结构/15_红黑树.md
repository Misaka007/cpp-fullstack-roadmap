...
**原理：** 设局部结构为 `P` 的右孩子 `S`，且 `S` 的左子树为 `B`。左旋后 `S` 上升，`P` 变成 `S` 的左孩子，`B` 改挂到 `P` 的右边。由于所有 `B` 中的键都满足 `P < B < S`，重新连接后 BST 顺序仍正确。红黑树插入的新结点先染红，若出现红父红子，再结合叔叔结点颜色选择变色或旋转；旋转负责调整形状，染色负责恢复红黑性质。
