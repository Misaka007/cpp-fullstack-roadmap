...
**原理：** BST 的每个结点都把候选值分成两部分：小于当前值的只能在左边，大于当前值的只能在右边。例：根为 `50`，插入 `30` 时先比较 `30 < 50`，进入左子树；再遇到 `20` 时比较 `30 > 20`，进入右子树；直到遇到空指针才插入。新结点总是叶子，不会移动原结点，因此插入后仍满足左小右大的规则。查找走的是同一条比较路径；树越接近平衡，路径越短。
