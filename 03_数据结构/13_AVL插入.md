...
**原理：** 插入新结点只会让“从插入点到根”的那一条路径高度发生变化，因此无需检查整棵树。例：插入 `30、10、20` 时，`20` 落在 `30` 的左子树、`10` 的右子树，形成 LR 型：先对 `10` 左旋，变成直线结构，再对 `30` 右旋。沿路径回溯时，平衡因子从 `0` 变为 `±1` 表示高度增加；变为 `±2` 表示首次失衡，需要按 LL、RR、LR、RL 选择旋转。
...
```text
算法 AVLInsert(root, key)
    按 BST 规则插入 key，并记录从根到插入点的路径
    从插入点的父结点开始向上回溯
        根据插入发生在左/右子树更新平衡因子
        如果平衡因子为 0
            停止回溯                        // 子树高度未继续增加
        如果平衡因子为 ±2
            按 LL、RR、LR、RL 类型旋转并停止
        结束如果
    结束回溯
结束算法
```
