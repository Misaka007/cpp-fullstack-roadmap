...
2. 递归创建表达清晰，但深度过大时可能造成栈溢出；应根据树深度和调用栈限制选择递归或显式栈实现。
...
以下代码使用递归创建二叉树；关键是明确空结点标记与左右子树的构造顺序。
...
先序遍历可以直接理解为先处理根结点。每遇到一个非空结点，先输出它自己，再完整处理左子树，最后完整处理右子树。左、右子树仍是一棵二叉树，所以可以调用同一个函数，这就是递归。
...
处理 A 时先输出 A；随后进入 B，输出 B 后继续处理 D、E；左子树结束后，递归调用自动返回 A，再处理 C。空指针表示这里没有子树，直接返回即可。

原理分三步：

1. node 为空时立即返回，这是递归的停止条件。
2. 输出语句在递归调用之前，所以根最先被访问。
3. 先调用左孩子、后调用右孩子，因此顺序固定为根、左、右。
...
```text
             ...
             +---+---+
                 ...
                 v
        +------------------+
        | node == nullptr? |  <-- 当前子树是否为空
        +----+--------+----+
          是 |        | 否
             v        v
  +------------------+  +------------------+
  | RETURN           |  | 访问 node.data   |  <-- 先访问根结点
  +--------+---------+  +--------+---------+
           |                     |
           v                     v
       +-------+        +-----------------------+
       |  END  |        | PreOrder(node->left)  |  <-- 再完整处理左子树
       +-------+        +-----------+-----------+
                                    |
                                    v
                         +------------------------+
                         | PreOrder(node->right)  |  <-- 最后处理右子树
                         +-----------+------------+
                                     |
                                     v
                                 +-------+
...
| 维度 | 回答要点 | 面试高频追问 |
| --- | --- | --- |
| 核心定义 | `二叉树` 属于数据结构与算法，关注数据表示、不变量和操作复杂度之间的取舍。 | 它解决什么问题，前置条件是什么？ |
...
- **问题：** “请说明 `二叉树` 的核心不变量，并描述一个最容易出错的边界场景。”
