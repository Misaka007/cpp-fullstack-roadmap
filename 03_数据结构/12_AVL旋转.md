...
**原理：** 以连续插入 `30、20、10` 为例：`30` 的左子树高度变成 2，右子树高度为 0，出现 LL 型失衡。右旋后 `20` 上升为根，`10` 在左、`30` 在右，树高恢复平衡。旋转并不是随意交换值，而是重连三棵局部子树：原左孩子的右子树交给原根的左侧，原根成为新根的右孩子。因此中序结果仍是 `10、20、30`，BST 的有序性不会改变。
...
算法 RotateRight(root)
    pivot ← root.left
    root.left ← pivot.right                // pivot 的右子树接到原根左侧
    pivot.right ← root                     // 原根下沉为 pivot 的右孩子
    更新 root 与 pivot 的高度/平衡因子
    返回 pivot
结束算法
```
