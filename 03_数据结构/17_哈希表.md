...
**原理：** 哈希函数把键转换成数组下标，例如桶数为 5、函数为 `key % 5`，键 `12` 和 `17` 都会映射到桶 2，这就是冲突。开链法不让后来的值覆盖前面的值，而是在桶 2 中保存链表 `12 -> 17`。查找 `17` 时先计算桶 2，再只比较该桶中的结点；当键分布均匀、桶数量合适时，链表很短，平均查找接近 `O(1)`。
...
```text
        ...
        +----+--------+----+
          否 |        | 是  <-- 条件不满足时走返回路径
             v        v
  +------------------+  +------------------+
  | RETURN NOT_FOUND |  | 比较当前结点 key |
  +--------+---------+  +--------+---------+
           |                     |
           v                     v
       +-------+        +------------------+
       |  END  |        | 匹配则返回，未匹配继续 next |
       +-------+        +--------+---------+
                                  |
                                  v
                         +------------------+
