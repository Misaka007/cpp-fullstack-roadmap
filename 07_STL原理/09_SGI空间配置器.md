...
```text
allocate(n)
  |-- n > threshold --> 一级配置器（malloc/free）
  +-- n <= threshold -> 二级配置器（自由链表）
                            |
                         内存池
```
...
| 维度 | 回答要点 | 面试高频追问 |
| --- | --- | --- |
| 核心定义 | `SGI空间配置器` 属于STL 原理，重点是泛型容器、迭代器、算法与分配器的协作模型。 | 它解决了什么问题，边界在哪里？ |
...
1. **基础：** 请定义 `SGI空间配置器`，并用本节示例说明它解决的实际问题。
...
```cpp
// 面试考点：这里说明 `SGI空间配置器` 的职责边界，而不是重复代码字面含义。
...
- **可能的面试题：** “请结合 `SGI空间配置器` 说明设计/实现选择，并给出一个失败场景。”
