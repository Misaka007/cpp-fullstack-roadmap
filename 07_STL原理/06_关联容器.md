...
| 容器类别 | 底层结构 | 查找复杂度 | 顺序 | 关键约束 |
| --- | --- | --- | --- |
| `map/set` | 平衡树 | O(log n) | 按比较器有序 | 比较器满足严格弱序 |
| `unordered_map/set` | 哈希表 | 平均 O(1)，最坏 O(n) | 无序 | `hash` 与 `equal_to` 一致 |
| `multimap/multiset` | 平衡树 | O(log n) | 允许重复键 | 用 `equal_range` 获取键区间 |

- `operator[]` 仅适用于非 `const map`，键不存在时会插入默认值；仅查询时使用 `find`、`contains` 或 `at`。
- 修改键会破坏树或哈希桶不变量；键在容器内应视为不可变。需要更新时删除后重新插入。
- 哈希容器扩容会使迭代器失效；可通过 `reserve` 和 `max_load_factor` 控制重哈希频率。
...
| 维度 | 回答要点 | 面试高频追问 |
| --- | --- | --- |
| 核心定义 | `关联容器` 属于STL 原理，重点是泛型容器、迭代器、算法与分配器的协作模型。 | 它解决了什么问题，边界在哪里？ |
...
1. **基础：** 请定义 `关联容器`，并用本节示例说明它解决的实际问题。
...
```cpp
// 面试考点：这里说明 `关联容器` 的职责边界，而不是重复代码字面含义。
...
- **可能的面试题：** “请结合 `关联容器` 说明设计/实现选择，并给出一个失败场景。”
